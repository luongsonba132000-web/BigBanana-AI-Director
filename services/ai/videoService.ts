...
/**
 * Veo 3.1 Fast：使用异步API生成视频（支持首尾帧，input_reference[] 数组）
 */
const generateVideoWithVeoFast = async (
  prompt: string,
  startImageBase64: string | undefined,
  ...
  const appendReference = async (base64: string, filename: string) => {
  ...
  if (startImageBase64) {
    await appendReference(startImageBase64, 'reference-start.png');
  }

  if (endImageBase64) {
    await appendReference(endImageBase64, 'reference-end.png');
  }
  ...
  console.log('📋 veo_3_1-fast 任务已创建，任务ID:', taskId);
  ...
  while (Date.now() - startTime < maxPollingTime) {
    ...
    console.log('🔄 veo_3_1-fast 任务状态:', status, '进度:', statusData.progress);
  ...
  console.log('✅ veo_3_1-fast 视频生成完成，视频ID:', videoId);
    ...
    try {
      ...
      if (contentType && contentType.includes('video')) {
        ...
        return new Promise<string>((resolve, reject) => {
          ...
          reader.onloadend = () => {
            const result = reader.result as string;
            console.log('✅ veo_3_1-fast 视频已转换为base64格式');
            resolve(result);
          };
          reader.onerror = () => reject(new Error('视频转base64失败'));
          reader.readAsDataURL(videoBlob);
...
export const generateVideo = async (
  ...
  // 异步模式
  if (isAsyncMode) {
    if (requestModel === 'veo_3_1-fast') {
      return generateVideoWithVeoFast(
        prompt,
        startImageBase64,
        endImageBase64,
        apiKey,
